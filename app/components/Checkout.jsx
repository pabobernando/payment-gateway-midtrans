...
const Checkout = () => {
  ...
  const generatePaymentLink = async () => {
    ...
    if (!secret) {
      console.error('NEXT_PUBLIC_SECRET is not defined');
      return;
    }
  ...
  return (
    <>
      <div className="flex items-center justify-between">
        ...
        <button
          className="rounded bg-cyan-500 p-4 text-sm font-medium transition hover:scale-105"
          onClick={checkout}
        >
          Checkout
        </button>
      ...
      <Link href={paymentUrl}>payment gas</Link>
      </div>
